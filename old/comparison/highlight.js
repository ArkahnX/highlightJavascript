...
highlight.strToXHTML = function(str)
{
	var addLen = 0;

	for (var key in highlight.xmlEntities)
	{
		str = str.replace(new RegExp(key, "g"),
			function(match, offset, s)
			{
				addLen += (highlight.xmlEntities[key].length - 1);
				return highlight.xmlEntities[key];
			}
		);
	}
	return {
...
highlight.copyObject = function(ob)
{
	var newOb = {};
	for (var prop in ob) {
		newOb[prop] = ob[prop];
...
highlight.highlightDocument = function(showLineNumbers)
{
	var codeList = document.getElementsByTagName("code");
	for (var i = 0, len = codeList.length; i < len; i++)
	{
		if (codeList[i].className && highlight.language[codeList[i].className])
		{
			highlight.highlight(codeList[i], showLineNumbers);
		}
	}
};

highlight.highlight = function(codeEl, showLineNumbers)
{
	var lang = highlight.language[codeEl.className];
	if (!lang)
	{
		return;
	}
	...
	for (var lineIndex = 0, lineCount = lines.length; lineIndex < lineCount; lineIndex++)
	{
		...
		for (var charIndex = 0, lineLen = line.length; charIndex < lineLen; charIndex++)
		{
			...
			if (beginMultiCommentIndex != -1)
			{
				...
				var substrOb = highlight.strToXHTML(line.substr(beginMultiCommentIndex, realEndIndex - beginMultiCommentIndex + 1));
			...
			if (lang.comment.single && c == lang.comment.single.start.charAt(0))
			{
				...
				if (beginSingleCommentIndex != -1)
				{
					...
					var substrOb = highlight.strToXHTML(line.substr(beginSingleCommentIndex));
					//var addLen = "<span class='comment'></span>".length;
			...
			if (c == "\"" || c == "'")
			{
				...
				for (charIndex += 1; charIndex < lineLen; charIndex++)
				{
					...
					{
						// Einde
						var substrOb = highlight.strToXHTML(line.substr(beginQuoteIndex, charIndex - beginQuoteIndex + 1));
			...
			{
				c = (highlight.xmlEntities[c]) ? highlight.xmlEntities[c] : c;
		...
		for (var i = 0; i < lang.keyword.length; i++)
		{
			...
			while (keywordIndex != -1)
			{
				...
				if (isKeyword) {
					...
					prop = highlight.copyObject(tmpOb);
